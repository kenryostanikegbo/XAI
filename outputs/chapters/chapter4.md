...
This chapter reports the analytical findings of the present study. The four research questions and six research objectives set out in Chapter 1 Section 1.5 are answered in turn, and the methodological commitments of Chapter 3 are honoured at every step. The chapter is the empirical centre of the dissertation: Chapter 1 motivates the work, Chapter 2 surveys the literature, Chapter 3 documents the methodology, and Chapter 5 turns the findings into conclusions and recommendations.

The chapter is organised in two parts. The first part (Section 4.1 Introduction, Section 4.2 Dataset Recap, Section 4.3 Methodology Recap, Section 4.4 SHAP as the Interpretability Framework) sets up the notation and definitions used throughout the analytical sections. The second part (Section 4.5 Predictive Performance Comparison, Section 4.6 Global SHAP Interpretability, Section 4.7 Local SHAP Interpretability, Section 4.8 Cross-Model Interpretability Comparison, Section 4.9 Synthesis and Discussion of Findings) reports the findings. Figures are numbered consecutively across the chapter from Figure 1 to Figure 14; tables from Table 1 to Table 5.

The mapping to research objectives is as follows. Objective 1 (data integration) is addressed in Section 4.2 and Section 4.3. Objective 2 (model training) is addressed in Section 4.3. Objective 3 (predictive comparison) is addressed in Section 4.5. Objective 4 (global SHAP) is addressed in Section 4.6, with the SHAP framework itself explained in Section 4.4. Objective 5 (local SHAP) is addressed in Section 4.7. Objective 6 (cross-model comparison) is addressed in Section 4.8 and synthesised in Section 4.9.
...
After excluding Withdrawn students (whose final outcome is ambiguous), the modelling matrix contains 22,311 rows out of the original 32,593 in the released dataset. The exclusion removes approximately 21 per cent of the original population, which means the 31.4 per cent Fail prevalence reported below is computed on a non-representative subset of the original OULAD population; the prevalence among all students (including Withdrawn) is closer to 21 per cent. The binary target is constructed by mapping Pass and Distinction to 0 and Fail to 1 (Chapter 3 Section 3.5.5), giving 15,299 Pass-or-Distinction rows (68.6 per cent) and 7,012 Fail rows (31.4 per cent). The minority class is therefore one-third of the population; a modest imbalance that justifies SMOTE (Chawla et al., 2002) inside the pipeline but that does not require aggressive rebalancing at evaluation time.

The 21 base features fall into four groups: six demographic (gender, age band, highest education, region, plus two constructed indicators), seven virtual-learning-environment engagement (total clicks, distinct resources, active days, mean clicks per active day, total days, weighted mean clicks per resource, ratio of active days to total days), five assessment (number submitted, mean score, weighted mean score, mean days from due date to submission, late-submission indicator), and three course-context (module, presentation, presentation year). After one-hot encoding, the modelling matrix contains 44 columns.

A stratified 70/30 split with `random_state=42` produces a training set of 15,617 rows (10,711 Pass, 4,906 Fail) and a held-out test set of 6,694 rows (4,588 Pass, 2,106 Fail), preserving the 31.4 per cent prevalence in both splits. The test set is used exactly once, for the final evaluation in Section 4.5.

## 4.3 Methodology Recap

Two classifiers are trained on the integrated matrix: a Logistic Regression with L2 regularisation and the l-bfgs solver, and a Random Forest with 500 trees and maximum depth 15 (Chapter 3 Section 3.7). Both are wrapped in an `imblearn.pipeline.Pipeline` that applies SMOTE inside each cross-validation fold (k_neighbors=5, random_state=42), so no test-fold information leaks into the training of any individual fold (Chapter 3 Section 3.6.4). Both are tuned by five-fold stratified grid search with `scoring='f1'`, and the best estimator is refit on the full training set before evaluation.

The Logistic Regression grid covers `C ∈ {0.01, 0.1, 1, 10}`, with `penalty='l2'`, `solver='lbfgs'`, `class_weight=None`, `max_iter=1000`. The `class_weight=None` choice is deliberate: SMOTE handles class imbalance inside the pipeline, so `class_weight='balanced'` would double-count it (Chapter 3 Section 3.7.2). The best estimator uses `C=1.0`.

The Random Forest grid covers `n_estimators ∈ {200, 500}`, `max_depth ∈ {None, 15, 25}`, `min_samples_split ∈ {2, 5}`, with `class_weight=None`, `n_jobs=-1`, `random_state=42`. The best estimator uses `n_estimators=500`, `max_depth=15`.

Six metrics are reported in Section 4.5: accuracy, precision, recall, F1, ROC-AUC, and average precision (AP). F1 is the tuning metric; AP is more sensitive than ROC-AUC to performance on the minority class (Saito & Rehmsmeier, 2015). Accuracy is reported for completeness but is not the primary metric because the 31.4 per cent class imbalance makes accuracy insensitive to the operationally important errors (failing students missed); reporting six metrics rather than accuracy alone directly addresses the over-reliance on accuracy identified in the OULAD literature (Chapter 2). The test set is left at its original 31.4 per cent prevalence, so reported metrics reflect the imbalanced deployment condition rather than a balanced evaluation setting.

Two SHAP explainers are used: LinearSHAP for the Logistic Regression and TreeSHAP for the Random Forest (Lundberg et al., 2020). LinearSHAP returns values on the log-odds scale; TreeSHAP returns values on the probability scale. The TreeSHAP computation is performed on a 300-row stratified subsample of the test set to keep the computation tractable; LinearSHAP is performed on the full test set. The 300-row subsample is reused in Section 4.7 for the Random Forest local explanations. The stratified subsample preserves the 31.4 per cent Fail prevalence of the full test set, so the marginal feature distributions and the per-feature SHAP distributions are not distorted by the subsampling. The choice of 300 rows is operationally modest: TreeSHAP for the present Random Forest takes approximately 12 minutes on a standard laptop for 300 rows, and would take approximately 4 hours for the full test set of 6,694 rows, which is within the scope of a research project but not within the scope of an interactive iteration loop. The 300 rows therefore represent a deliberate trade-off between computational tractability and statistical precision.

The full pipeline is reproducible from a clean clone of the repository in under 45 minutes on a standard laptop (Chapter 3 Section 3.10). The random seed is fixed at every stochastic stage: the train/test split, the cross-validation shuffling, the SMOTE instance generation, and the Random Forest initialisation all use `random_state=42`. Every notebook can be re-run from top to bottom without manual intervention, and every figure and table cited in this chapter is generated by the corresponding notebook and saved to `outputs/figures/` or `outputs/tables/`. The end-to-end reproducibility check is documented in Chapter 3 Section 3.10 and is the basis on which the present chapter's claims can be audited by a future reader.

## 4.4 SHAP as the Interpretability Framework

SHAP (Lundberg & Lee, 2017) is the interpretability framework used for both classifiers. It is chosen over alternatives for three reasons: it is local (each prediction gets its own explanation), faithful (the SHAP values sum to the model's output), and defined for both linear and tree-ensemble models through specialised explainers (LinearSHAP and TreeSHAP). Permutation importance is faithful but not local; LIME (Ribeiro et al., 2016) is local but not consistent across runs; Logistic Regression coefficients are local and faithful but not model-agnostic. Only SHAP offers all three properties simultaneously.
...
For a linear model `f(x) = b + Σ w_i · x_i`, the SHAP value for feature `i` is `φ_i = w_i · (x_i − E[x_i])`, where the expectation is over the background dataset. The attribution is in log-odds units, and the sum of the base value and the SHAP values equals the model's prediction. For a Random Forest, TreeSHAP (Lundberg et al., 2020) computes exact SHAP values in polynomial time by exploiting the tree structure, returning attributions in probability units. The additivity property; that the SHAP values sum to the model's output minus the base value; is the foundation of the cross-model comparison in Section 4.8: the sign of each SHAP value (push toward Pass or toward Fail) is comparable across the two explainers, even though the magnitudes are not.

Two limitations bound the present SHAP analysis. First, SHAP values are statistical associations, not causal effects (Chapter 2 Section 2.4): a feature with a large positive SHAP value is associated with the model's prediction of Fail, not necessarily a feature whose manipulation would change the prediction. Second, SHAP values are computed with respect to the model's prediction, not the true outcome: a feature that the model uses heavily may be a feature the model has learned to overweight rather than a feature genuinely informative about the outcome. Both limitations are flagged when the present findings are read.

The SHAP plots in Section 4.6 (beeswarm and bar chart) and Section 4.7 (waterfall) follow the conventions of the `shap` Python package. In a beeswarm plot, each dot is one test instance, the horizontal position is the SHAP value, and the colour encodes the feature value (red for high, blue for low). In a bar chart, each bar is the mean absolute SHAP value across the test set, sorted descending. In a waterfall plot, each row is one feature, the bar length is the SHAP value, the colour is red for positive and blue for negative, and the bottom row shows the model's final prediction. A further convention worth noting: when a feature is missing from the modelling matrix for a given instance (which does not occur in the present study, but is common in production settings), the SHAP value for that feature is by definition zero, satisfying the missingness property. When the same instance is explained under two different background datasets, the SHAP values differ in magnitude but not in sign, and the local accuracy property holds under both. The present study uses the training set as the background dataset for LinearSHAP, which is the conventional choice and is consistent with the documentation of the `shap` Python package (Lundberg et al., 2020).
...
Both models identify `submission_rate` and `weighted_mean_score` as the strongest drivers of student failure, which gives a degree of cross-model corroboration to the engagement-based hypothesis. Beyond that overlap, the two models diverge. The Logistic Regression top ten is dominated by demographic and course-context features, while the Random Forest top ten is dominated by engagement and assessment metrics. The two top-ten lists share only four features, and the rank-ordering of those four features also differs. The detailed quantification of this divergence is reported in Section 4.8.

A note on the interpretation of the Logistic Regression beeswarm in Figure 3 is worth flagging at this point. The features with the widest per-instance spreads; the engagement metrics and the assessment metrics; are the features that vary most across students in absolute terms, but their per-instance SHAP contributions are tightly clustered around their mean values. This is a property of linear models: the SHAP contribution of a feature is proportional to the deviation of its value from the mean, so features with low variance contribute small SHAP values on every instance. The Random Forest beeswarm in Figure 5 shows the opposite pattern: the engagement metrics have wide per-instance spreads, indicating that the Random Forest uses them differently depending on the values of other features. The contrast between the two beeswarms is therefore not just a contrast in feature importance, it is a contrast in how each model uses the features; additive versus interactive.

## 4.7 Local SHAP Interpretability

This section addresses Research Question 4 by inspecting six individual cases; the four corners of the confusion matrix (TP, TN, FN, FP) plus two borderline students whose predicted probability lies in the 0.4–0.6 band. Twelve waterfall plots in total; six cases times two models; are presented as Figure 7 to Figure 12. The six cases are drawn from the 300-row stratified SHAP subsample so that every per-instance SHAP value is already computed.
...
Student 248511 in presentation 2013B (Figure 11) actually passed; the Logistic Regression predicts Fail at P(Fail) = 0.566 while the Random Forest predicts Pass at P(Fail) = 0.498. This is the case where the two models diverge most clearly, and the local explanation illustrates the difference in reasoning: the Logistic Regression leans on region and education features, with several smaller features pushing toward Fail; the Random Forest credits assessment score and submission rate. The two explanations are roughly mirror images: both end up near the decision boundary, but for different stated reasons.
...
Student 429212 in presentation 2013J (Figure 12) actually passed, and both models predict Pass but with low confidence: LR P(Fail) = 0.463, RF P(Fail) = 0.496. The two models agree on the label but for different feature-based reasons. The Random Forest credits three engagement metrics that almost fully cancel the base rate. The Logistic Regression explanation is again dominated by demographic features with one engagement feature tipping the prediction toward Pass. The two models reach the same decision at the boundary via different feature rankings.
...
The False Negative case deserves additional comment because it is the operationally most important case in the study. This is the case where the early-warning system fails at its primary task: catching failing students before it is too late to intervene. The student in this case had a positive engagement record; high submission rate, several assessments submitted, an assessment score in the passing range; and both models therefore predicted Pass with high confidence. The actual failure was invisible to the engagement-only lens: no engagement feature pointed toward Fail. This is a structural limitation of any model that relies on virtual-learning-environment engagement as the primary signal: a student who engages consistently but fails for some other reason (a final-exam mishap, an administrative flag, a personal crisis) will be systematically missed. The False Positive case, by contrast, illustrates the false-alarm cost: a student who was actually passing was flagged Fail by both models, with the Random Forest assigning a higher probability. The institution that uses the model to triage support would contact this student for an outreach conversation they did not need, which is a cost but a smaller one than the False Negative case. The two borderline cases show where the choice between models matters most: Borderline-1 is the one case in the sample where the two models disagree on the predicted label, and Borderline-2 is the one case where they agree on the label but for different reasons.

## 4.8 Cross-Model Interpretability Comparison

This section addresses Research Question 3 by quantifying and visualising the agreement and disagreement between the Logistic Regression and Random Forest global explanations. Two summary statistics anchor the comparison: the Spearman rank correlation between the two mean-|SHAP| vectors (Spearman, 1904), and the Jaccard index of the top-10 feature sets. Local-level agreement is then reported on the six illustrative cases from Section 4.7.
...
Logistic Regression and Random Forest identify substantially different factors as the drivers of student failure. The Spearman rank correlation is positive but not statistically distinguishable from zero, and the top-10 lists share only four features. The LR top-10 is dominated by demographic and course-context features; the RF top-10 is dominated by engagement and assessment features. At the local level, the two models agree in direction for the four corners of the confusion matrix but disagree for the two borderline cases near the decision boundary. The choice of model family changes both the global explanation and the local prediction: an early-warning system that runs Random Forest and one that runs Logistic Regression would flag somewhat different students and would offer somewhat different justifications.

A subtle point worth flagging: the cross-model divergence is partly a function of feature-engineering choices. Both models see the same 44 features, but Logistic Regression sees them on a standardised scale while Random Forest sees them on their original scale. The TreeSHAP and LinearSHAP values are not symmetric, and the disagreement may partly reflect a scale-induced reordering rather than a fundamental epistemic disagreement. A robustness check in which the Random Forest is also evaluated on the standardised features would clarify this, and that check is left for future work.

## 4.9 Synthesis and Discussion of Findings

This section synthesises the analytical findings of Section 4.5–Section 4.8 against the four research questions set in Chapter 1 Section 1.5 and the methodological commitments of Chapter 3. The aim is integrative rather than additive: the four preceding sections each address one research question in isolation, whereas this section reads them together to expose the higher-order pattern.

### 4.9.1 Cross-Cutting Theme: Accuracy versus Explanatory Coherence

The Random Forest leads Logistic Regression on five of six predictive metrics (accuracy, recall, F1, ROC-AUC, AP; Section 4.5), but the lead is modest; F1 gap of 0.020, ROC-AUC gap of 0.005; and is concentrated on recall. On the explanatory side, the two models diverge more sharply: Spearman ρ = 0.10 over the 44-feature importance vectors (Section 4.8), top-10 Jaccard = 0.25, and only four features appear in both top-10 lists. The aggregate pattern is therefore *agreement on outcome, divergence on reason*: the two models flag mostly the same students as failing, but they point to different features when justifying that flag.

### 4.9.2 Joint Reading of the Four Research Questions

Research Question 1 (predictive performance) is settled: the Random Forest is the marginally stronger classifier, and the gap rests on its higher recall on the minority class (Section 4.5). Research Question 2 (global feature importance) shows a clear two-track structure: Logistic Regression leans on demographic and course-context features, Random Forest leans on engagement and assessment features, and only `submission_rate`, `weighted_mean_score`, `edu_A Level or Equivalent`, and `gender_F` survive in both top-10 lists (Section 4.6, Section 4.8). Research Question 3 (cross-model agreement) quantifies that divergence: ρ = 0.10, Jaccard = 0.25, and a structural-versus-behavioural split in the LR-only and RF-only features (Section 4.8). Research Question 4 (local interpretability) shows that the two models agree on direction for the four corners of the confusion matrix but disagree on the two borderline cases near the decision boundary (Section 4.7, Section 4.8).

### 4.9.3 Operational Implications

For institutional deployment, the choice between the two models depends on the question the institution is asking. If the question is *which students are at risk?*, the Random Forest gives a slightly more complete answer (recall 0.831 versus 0.789, Section 4.5). If the question is *why are they at risk?*, the two models give different answers; the Logistic Regression explanation points to demographic and structural factors, the Random Forest explanation points to engagement and assessment factors (Section 4.6, Section 4.7). A useful early-warning system would not commit to a single model family but would present both explanations side by side, letting a tutor triangulate. The borderline cases (Section 4.7, Section 4.8) are exactly the students where the two lenses disagree most strongly, and these are the students most in need of human judgement.

### 4.9.4 Methodological Limitations Visible in the Results

Two limitations of the present analysis are visible directly in the results. First, both models were tuned on F1 with a default 0.5 threshold; the apparent F1 lead depends on that threshold (Section 4.5). A threshold-sensitivity analysis would be needed to claim a stable performance gap. Second, the cross-model comparison is computed on the 300-row stratified SHAP subsample for the Random Forest (Section 4.6), so the RF global ranking reflects that subsample; the LR global ranking is computed over the full 6,694-row test set. Both limitations are minor and do not invalidate the qualitative findings.

### 4.9.5 Closing Note on Chapter 4

Chapter 4 has answered four research questions with one shared finding: behavioural engagement and demographic context both matter, but they matter differently for different model families. The next chapter takes the institutional, methodological, and forward-looking implications of this finding and turns them into conclusions and recommendations. The headline numbers to carry into Chapter 5 are these: ROC-AUC of 0.957 (LR) and 0.962 (RF); F1 of 0.840 (LR) and 0.859 (RF); Spearman ρ of 0.10 across the 44-feature importance vectors; top-10 Jaccard of 0.25; four shared top-10 features; six LR-only features (demographic and course-context); six RF-only features (engagement and assessment); local direction agreement of 4 of 6 cases; one borderline case where the two models disagree on the predicted label. These nine numbers are the empirical core of the dissertation.

A final methodological observation closes the chapter. The present study has chosen two classifiers that span the model-family spectrum; a linear model and a tree ensemble; and has used SHAP to put them on a comparable footing. An alternative design would have been to choose two classifiers within the same family; for example, two Random Forests with different hyperparameters, or a Logistic Regression and a Lasso; and the cross-model comparison would have been very different. The present design maximises the contrast between the two explanations, at the cost of confounding model family with feature scale (Section 4.8). A reader who wants to isolate the effect of model family would need to repeat the comparison with two classifiers from the same family, and that comparison is left for future work.
